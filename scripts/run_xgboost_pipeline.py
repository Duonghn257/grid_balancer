#!/usr/bin/env python3
"""
Script chạy toàn bộ pipeline XGBoost cho DiCE
Chạy: EDA -> Preprocessing -> Training -> Prediction
"""
...
def main():
    """Chạy toàn bộ pipeline XGBoost"""
    print("=" * 80)
    print("CHẠY TOÀN BỘ PIPELINE XGBOOST CHO DiCE")
    print("=" * 80)
    
    scripts = [
        ("01_eda_analysis.py", "EDA Analysis"),
        ("02_data_preprocessing.py", "Data Preprocessing"),
        ("06_train_xgboost_for_dice.py", "Train XGBoost Model"),
        ("07_predict_with_xgboost.py", "Prediction với XGBoost")
    ]
    ...
    if all_success:
        print("\n🎉 Pipeline XGBoost hoàn thành thành công!")
        print("\n📁 Kết quả:")
        print("   - EDA: analysis/")
        print("   - Processed data: output/processed_data.parquet")
        print("   - Models: output/models/")
        print("     * xgboost_dice.pkl")
        print("     * xgboost_wrapped_dice.pkl (cho DiCE)")
        print("   - Predictions: output/predictions_xgboost.csv")
        print("\n✅ Model đã sẵn sàng cho DiCE integration!")
    else:
